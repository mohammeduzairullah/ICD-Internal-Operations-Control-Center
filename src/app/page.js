...
export default function HomeGateway() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-tr from-slate-950 via-slate-900 to-blue-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">

      {/* Structural Motion Ambient Lighting */}
      <div className="absolute top-1/4 left-1/4 w-80 h-80 bg-cyan-500/10 blur-[100px] rounded-full animate-[pulse_6s_ease-in-out_infinite] pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-indigo-500/10 blur-[120px] rounded-full animate-[pulse_8s_ease-in-out_infinite_1.5s] pointer-events-none"></div>

      {/* Control Module Card Structure */}
      <div className="max-w-md w-full bg-slate-900/70 backdrop-blur-3xl border border-slate-700/60 rounded-3xl p-8 shadow-[0_0_50px_rgba(6,182,212,0.1)] relative z-10 hover:border-cyan-500/40 hover:shadow-[0_0_60px_rgba(6,182,212,0.2)] transition-all duration-500">
        ...
        <div className="grid grid-cols-3 border border-slate-800 bg-slate-950/90 p-1.5 rounded-2xl mb-8 font-mono text-[10px] font-bold tracking-wider">
          <button
            type="button"
            onClick={() => { setActiveTab('login'); setMessage(''); setTrackingResult(null); }}
            className={`py-2.5 rounded-xl transition-all duration-300 ${activeTab === 'login' ? 'bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
          >
            SIGN IN
          </button>
          <button
            ...
            className={`py-2.5 rounded-xl transition-all duration-300 ${activeTab === 'register' ? 'bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
          >
            NEW SELLER
          </button>
          <button
            ...
            className={`py-2.5 rounded-xl transition-all duration-300 ${activeTab === 'track' ? 'bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
          >
            TRACK CARGO
          </button>
        </div>
        ...
        <div className="text-center mb-6" style={{ animation: 'fadeIn 0.3s ease-out' }} key={activeTab}>
          <h1 className="text-xl font-black tracking-[0.25em] text-white uppercase font-sans">
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-400 transition-all font-mono placeholder:text-slate-700 focus:ring-4 focus:ring-cyan-500/5 shadow-inner"
              />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 font-mono">
          ...
          <form onSubmit={handleTrackLookup} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-400 transition-all font-mono uppercase tracking-widest placeholder:text-slate-700 shadow-inner"
              />
            </div>

            {message && (
              <div className="text-[10px] font-mono text-center text-rose-400 p-3 bg-slate-950 rounded-xl border border-slate-800">
                {message}
              </div>
            )}

            {trackingResult && (
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl font-mono text-xs space-y-2.5 text-slate-300" style={{ animation: 'slideIn 0.25s ease-out' }}>
                <div className="flex justify-between border-b border-slate-900/60 pb-1.5">
                  <span className="text-slate-500">REF ID:</span>
                  <span className="font-bold text-white tracking-wider">{trackingResult.id}</span>
                </div>
                <div className="flex justify-between border-b border-slate-900/60 pb-1.5 items-center">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-400 text-[11px]">{new Date(trackingResult.gate_in_time).toLocaleString()}</span>
                </div>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-cyan-500 via-indigo-500 to-blue-600 hover:opacity-95 text-slate-950 font-black py-3.5 rounded-xl transition-all font-mono text-xs uppercase tracking-widest mt-1 active:scale-[0.98] disabled:opacity-50"
            >
